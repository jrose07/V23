...
Aus den Messwerten wird also wieder durch \textit{find\_peaks}~\cite{scipy} die Amplitude $A(\Theta)$ für beide aufgespalteten Resonanzfreqzenten bestimmt und mittels \textit{curve\_fit}~\cite{scipy} an die Kugelflächenfunktion $Y_{lm}(\Theta)$ gefittet. $m$ wird diesmal einfach auf $0$ gesetzt, da die Abhängigkeit von $\varphi$ sowieso nicht mehr gemessen wird. \\ 
