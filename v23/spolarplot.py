...
def fitfunc(alpha, A, b, c, l, m):
    alpha = alpha - c
    theta = np.arccos(-1/2*(1-np.cos(alpha)))
    phi = np.arcsin(1/np.sqrt(2)*np.sin(alpha))
    return A*np.real(sph_harm_y(l,m,theta,phi)) + b
...
def spolarplot(folder, name, l, m, *args, **kwargs):
    ...
    ax.plot(alpha_fit, fitfunc(alpha_fit, *popt, l=bl, m=bm), "r--", label=f"Theoriefit mit Ordnung {bl}, {bm}")
...
spolarplot("raw/peak4-9", "peak4-9.pdf",4, 0)
